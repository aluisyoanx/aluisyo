...
bool processServerAliasResponse(const std::string& s, std::string& address) {
  ...
  // Courtesy of Monero Project
		// make sure the txt record has "oa1:aNX" and find it
		auto pos = s.find("oa1:aNX");
		if (pos == std::string::npos)
			return false;
...
bool simple_wallet::init(const boost::program_options::variables_map& vm) {
  ...
  if (m_generate_new.empty() && m_wallet_file_arg.empty()) {
    ...
    std::cout << "Specify the wallet file name (e.g., name.wallet).\n";
...
bool simple_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	...
	try {
		...
		const std::string filename = "reserve_proof_" + args[0] + "_ANX.txt";
...
int main(int argc, char* argv[]) {
  ...
  bool r = command_line::handle_error_helper(desc_all, [&]() {
    ...
    if (command_line::get_arg(vm, command_line::arg_help)) {
      ...
      std::cout << "Aluisyo Wallet v" << PROJECT_VERSION_LONG << std::endl;
      std::cout << "Usage: aluisyowallet [--wallet-file=<file>|--generate-new-wallet=<file>] [--daemon-address=<host>:<port>] [<COMMAND>]";
  ...
  logger(INFO, BRIGHT_GREEN) << "Aluisyo Wallet v" << PROJECT_VERSION_LONG;
